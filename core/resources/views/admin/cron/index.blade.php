...
@section('panel')
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu p-0">
                                                    <a href="{{ route('cron') }}?alias={{ $cron->alias }}"
                                                        class="dropdown-item"><i class="las la-check-circle"></i>
                                                        @lang('Run Now')</a>
                                                    @if ($cron->is_running)
                                                        <a href="{{ route('admin.cron.schedule.pause', $cron->id) }}"
                                                            class="dropdown-item"><i class="las la-pause"></i>
                                                            @lang('Pause')</a>
                                                    @else
                                                        <a href="{{ route('admin.cron.schedule.pause', $cron->id) }}"
                                                            class="dropdown-item"><i class="las la-play"></i>
                                                            @lang('Play')</a>
                                                    @endif
                                                    <a href="" data-id="{{ $cron->id }}"
                                                        data-name="{{ $cron->name }}" data-url="{{ $cron->url }}"
                                                        data-next_run="{{ $formattedDateTime }}"
                                                        data-cron_schedule_id="{{ $cron->cron_schedule_id }}"
                                                        data-default="{{ $cron->is_default }}"
                                                        class="dropdown-item updateCron"><i class="las la-pen"></i>
                                                        @lang('Edit')</a>
                                                    <a href="{{ route('admin.cron.schedule.logs', $cron->id) }}"
                                                        class="dropdown-item"><i class="las la-history"></i>
                                                        @lang('Logs')</a>
                                                    @if (!$cron->is_default)
                                                        <a href="javascript:void(0)"
                                                            data-action="{{ route('admin.cron.delete', $cron->id) }}"
                                                            data-question="@lang('Are you sure to delete this cron?')"
                                                            class="dropdown-item confirmationBtn"><i
    ...
    <div class="modal fade" id="addCron">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="form-horizontal disableSubmission resetForm" method="post"
                    action="{{ route('admin.cron.store') }}">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <select name="cron_schedule_id" class="form-control select2"
    ...
    <div class="modal fade" id="updateCron">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
...
@push('breadcrumb-plugins')
    <button type="btn" class="btn btn-outline--primary addCron"><i
            class="las la-plus"></i>@lang('Add')</button>
    <a href="{{ route('admin.cron.schedule') }}" class="btn btn-outline--info"><i
            class="las la-clock"></i>@lang('Cron Schedule')</a>
@endpush
