...
<nav class="navbar-wrapper bg--dark d-flex flex-wrap">
    <div class="navbar__left">
        <button type="button" class="sidebar-open-btn me-3"><i class="las la-bars"></i></button>
        <button type="button" class="res-sidebar-open-btn me-3"><i class="las la-bars"></i></button>
        <button type="button" style="background-color: transparent; color: white; font-size: 23px;" class="exchange-alert-btn me-3"><i class="las la-bell"></i></button>
        <form class="navbar-search disableSubmission">
...
<script>
    ...
    // sideber toggle
    const sidebar_toggle_button = document.querySelector('.sidebar-open-btn');
    // let sidebar_show = Boolean(localStorage.getItem('sidebar_show'))
    let sidebar_show = 1
    const body_wrapper = document.querySelector('.body-wrapper');
    const navbar_wrapper = document.querySelector('.navbar-wrapper');
    const sidebar = document.querySelector('.sidebar');
    sidebar_toggle_button.addEventListener('click',(e) => {
        // margin-left: 250px;
        if(sidebar_show){
            body_wrapper.style.marginLeft = '0px';
            navbar_wrapper.style.marginLeft = '0px';
            sidebar.style.display = 'none';
            sidebar_show = 0;
            // localStorage.setItem('sidebar_show', 0)
        } else {
            body_wrapper.style.marginLeft = '250px';
            navbar_wrapper.style.marginLeft = '250px';
            sidebar.style.display = 'flex';
            sidebar_show = 1;
            // localStorage.setItem('sidebar_show', 1)
        }
    })
    const exchange_alert_btn = document.querySelector('.exchange-alert-btn');
    let exchange_alert_btn_status = false;
    document.addEventListener('click', () => {
        exchange_alert_btn.style.color = 'forestgreen';
        exchange_alert_btn_status = true;
    });
</script>
@endpush
