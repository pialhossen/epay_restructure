...
@section('panel')
    ...
    @if (@$section->element)
        ...
        {{-- Update METHOD MODAL --}}
        <div id="updateBtn" class="modal fade">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
    ...
    <div class="modal fade" id="importConfirmationModal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <h6 class="mb-2">@lang('If you import content from another template')</h6>
                    <p class="text--danger">
                        * @lang('Current data will be removed.')
                    </p>
                    <p class="text--danger">
                        * @lang('Images will not import')
                    </p>
                    <p class="text--danger">
                        * @lang('You need to re-upload the images.')
                    </p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn--dark" data-bs-dismiss="modal">@lang('Cancel')</button>
                    <button type="button" class="btn btn--primary" id="importData">@lang('Import')</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="d-flex flex-wrap justify-content-end gap-2 align-items-center">
        ...
        @if (!empty($templates))
            <div class="form-inline float-sm-end">
                <form action="{{ route('admin.frontend.import', $key) }}" method="post" class="disableSubmission" id="importForm">
                    <div class="input-group">
                        @csrf
                        <select name="template_name" class="form-control form-control-sm border--primary h-auto select2"
                            data-minimum-results-for-search="-1">
                        ...
                        <button type="submit" class="input-group-text btn btn-sm btn--primary"
                            id="importContent">@lang('Import')</button>
                    </div>
                </form>
            </div>
        @endif
...
@push('script')
    <script>
        (function($) {
            ...
            $('#importContent').on('click', function(e) {
                e.preventDefault();
                let modal = $('#importConfirmationModal');
                let templateName = $('[name=template_name]').val();

                if (!templateName) {
                    notify('error', 'Plese select a template');
                    return;
                }
                modal.find('.modal-title').text(`@lang('Import contents from ${templateName}') template`);
                modal.modal('show');
            });
