...
class ForwardEmailController extends Controller
{
    ...
    {
        if ($this->user->id == 1 || $this->user->is_superadmin || $this->user->can('Update - Exchange Emails')) {
            return 0;
        }
        abort(403);
    }
    ...
    {
        if ($this->user->id == 1 || $this->user->is_superadmin || $this->user->can('View - Exchange Emails Details')) {
            return 0;
        }
        abort(403);
    }
    public function index(Request $request)
    {
        ...
        $emails_query = ForwardEmail::where('is_hidden',false)->where('is_archive',false);
        if($request->id){
        ...
        if($request->note){
            $emails_query = $emails_query->where('note','LIKE', "%$request->note%");
        }
    ...
    {
        $pageTitle = "Exchange Archive Emails";
        $emails_query = ForwardEmail::where('is_hidden',false)->where('is_archive',true);
        if($request->id){
            $emails_query = $emails_query->where('id', $request->id);
        }
        if($request->from){
        ...
        $emails = $emails_query->with('checked_by_admin')->latest()->paginate(getPaginate($request->itemsPerPage ? $request->itemsPerPage : null));
        return view('admin.exchange_emails.archive_list', compact('emails','request','pageTitle'));
    }
    ...
    public function archive(ForwardEmail $email, Request $request){
        $this->check_update_permission();
        $email->is_archive = true;
        $email->save();
        $notify[] = ['success', 'Email Archive'];
        return back()->withNotify($notify);
    }
    public function unarchive(ForwardEmail $email){
        $this->check_update_permission();
        $email->is_archive = false;
        $email->save();
        $notify[] = ['success', 'Email Unarchive'];
        return back()->withNotify($notify);
    }
}
