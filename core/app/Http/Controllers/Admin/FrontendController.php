...
class FrontendController extends Controller
{
    ...
    public function templates()
    {
        ...
        $templateBasePath = base_path('resources/views/templates');
        $temPaths = array_filter(glob($templateBasePath.'/*'), 'is_dir');

        Log::info('Template directories:', $temPaths);

        foreach ($temPaths as $key => $temp) {
            $tempname = basename($temp);
            $templates[$key]['name'] = $tempname;
            $templates[$key]['image'] = asset("templates/$tempname/preview.jpg"); // asset() works with `public/`, so adjust if needed
        }
    ...
    public function frontendSections($key)
    {
        ...
        // 🔧 Fixed template directory scanning
        $allTemplatePaths = glob(base_path('resources/views/templates/*')) ?: [];

        // 🔎 Filter only directories
        $templateDirs = array_filter($allTemplatePaths, 'is_dir');

        // ❌ Exclude currently active template
        $activeTemplate = gs('active_template');
        $temPaths = array_filter($templateDirs, function ($path) use ($activeTemplate) {
            return basename($path) !== $activeTemplate;
        });

        // 🔄 Collect matching templates that contain this section
        $templates = [];

        foreach ($temPaths as $tempKey => $path) {
            $templateName = basename($path);

            $templateSections = json_decode(
                json_encode(getPageSections(false, "templates/$templateName/")),
                true
            );

            if (array_key_exists($key, $templateSections)) {
                $templates[$tempKey]['name'] = $templateName;
            }
        }

        return view('admin.frontend.section', compact(
            'section',
            'content',
            'elements',
            'key',
    ...
    //        $pageTitle = $section->name;
    //
    //        $temPaths = array_filter(base_path('resources/views/templates/*'), 'is_dir');
    //        $temPaths = array_diff($temPaths, ["resources/views/templates/" . gs('active_template')]);
    //        $templates = [];
